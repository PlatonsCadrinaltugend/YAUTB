 ...
 * - text: message
 * @typedef {{messages:Boolean, channel:String, socket:Websocket, text:String}}
 *  */
exports.send = function send(messages, channel ,socket, text){
	if (messages){
...
exports.isMod = async function isMod(channel, userid){
	let ml = await fetch(`https://api.modscanner.com/twitch/channel/${channel}`).then(data => data.json());
	if (!ml){
		return false;
	}
	if(ml.id == userid){
