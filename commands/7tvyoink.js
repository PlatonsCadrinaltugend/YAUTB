exports.execute = {
    ...
    code:(async function stvadd(args, standartargs){
        ...
        if(mes.length<3){
            util.send(args.messages, args.originChannel, standartargs.socket,'ERROR: Missing Arguments');
        }
        let name = mes[2];
        if (name == null){
        ...
        util.send(args.messages, args.originChannel, standartargs.socket,'Trying to yoink emote Loading');
